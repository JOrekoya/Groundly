...
#: Assessor characteristics are published per assessment year. Joining against
#: an old year silently loses recent construction.
...
def parse_sale(row: dict[str, Any]) -> Sale | None:
    ...
    if not pin or sale_date is None or not price or price <= 0:
        return None
    return Sale(parcel_id=pin, sale_date=sale_date, sale_price=price)
...
def parse_location(row: dict[str, Any]) -> Location | None:
    ...
    return Location(
        ...
        latitude=lat,
        longitude=lon,
        area_code=row.get("township_code"),
    )
...
class CookCountyAdapter:
    """Satisfies :class:`~app.ingestion.adapter.CountyAdapter`.

    County-specific knobs live here rather than in the protocol: the assessment
    year to join characteristics against, and an optional township to narrow
    to.
    """
    ...
    def __init__(
        ...
        *,
        assessment_year: int = DEFAULT_ASSESSMENT_YEAR,
        township_code: str | None = None,
    ) -> None:
        self.client = client
    ...
    @property
    def name(self) -> str:
        if self.township_code:
            return f"{COUNTY_NAME} (township {self.township_code})"
        return COUNTY_NAME
    ...
    def count_sales(
        ...
        where = sales_where(
            since, township_code=self.township_code, property_type=property_type
        )
        rows = self.client.query(SALES_DATASET, select="count(1)", where=where)
    ...
    def fetch_sales(
        ...
        *,
        ...
        Sorting by date and taking the first N returns only the most recent
        slice, which is not a sample of the window — it is a sample of last
        quarter. That matters because assessor characteristics are published
        for a single assessment year, so join rates decay slightly as sales
        age, and a recency-biased sample reports a coverage figure the full
        window will not match.

        ``stratify`` spreads the pull evenly across the calendar months of the
        ...
        if not stratify:
            rows = pull(
                sales_where(
                    since,
                    township_code=self.township_code,
    ...
    def fetch_comp_sales(
        ...
        *,
        ...
        """Pull sales and join them to characteristics and location.

        Sales come first, then the other three datasets are queried only for
        the PINs that actually sold, which keeps request volume proportional to
        the window rather than to the size of the county.
        """
        ...
        pins = sorted({sale.parcel_id for sale in sales})
        characteristics: dict[str, Characteristics] = {}
        locations: dict[str, Location] = {}

        for batch in _chunk(pins, PIN_BATCH_SIZE):
            clause = f"{pin_in_clause(batch)} AND year = {self.assessment_year}"
            ...
            for row in self.client.query(
                ...
                if parsed is not None:
                    characteristics.setdefault(parsed.parcel_id, parsed)

            for row in self.client.query(
                CONDO_DATASET,
                select="pin, char_bedrooms, char_unit_sf, char_land_sf, char_yrblt",
                where=(
                    f"{clause} AND is_parking_space = false "
            ...
            for row in self.client.query(
                ...
                if parsed is not None:
                    locations.setdefault(parsed.parcel_id, parsed)

        return join_sales(sales, characteristics, locations, county=COUNTY_NAME)
